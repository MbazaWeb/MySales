-- DukaVerse — Migration v2
-- Adds cost_price, selling_price to products
-- Adds profit, customer_name, customer_phone to sales
-- Updates record_sale() RPC to handle new fields
-- Run this AFTER the baseline schema is applied.
...
-- Backfill: existing products → cost_price = price * 0.8, selling_price = price
...
begin
  ...
  v_unit_price := case when v_product.selling_price > 0 then v_product.selling_price else v_product.price end;
...
-- ─────────────────────────────────────────────
--  4. Update grant for products (new columns)
-- ─────────────────────────────────────────────
grant update (name, sku, category, unit, price, cost_price, selling_price, reorder, is_active) on public.products to authenticated;

-- ─────────────────────────────────────────────
--  5. Update low_stock view to include new price cols
-- ─────────────────────────────────────────────
create or replace view public.low_stock
with (security_invoker = true)
as
select id, branch_id, name, sku, category, stock, unit,
       price, cost_price, selling_price, reorder, updated_at
from public.products
where is_active and stock <= reorder;

-- Done. Verify:
-- select name, cost_price, selling_price, (selling_price - cost_price) as profit_per_unit from public.products limit 5;
